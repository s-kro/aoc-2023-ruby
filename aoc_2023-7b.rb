...
File.readlines('aoc_2023-7.dat').each do |hb| # group into same cards,
  hand, bid = hb.split(' ') # ... join like cards together, then sort the 'of a kinds' to first pos by no of cards
  ...
  (0..sh.length - 1).each do |i|
    if (sh[i].chars.first == 'J' && sh.length > 1) then # check against 5 jokers, no sense doing anything
      ...
      break                                          #  ... no longer matches the array (avoid looping 
    end                                              # ... though elements that no longer exist)
  end
